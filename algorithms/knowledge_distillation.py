...
class KnowledgeDistillationAgent:
    ...
    def trainNetwork(self, batch_experiences):
        ...
        # -----------------------------------------------------------
        # computing returns at once only
        returns = torch.zeros_like(rs).to(self.device)
        G = torch.zeros(B).to(self.device)
        for t_ in reversed(range(T)):
            G = rs[:, t_] + self.gamma * G * (1 - dones[:, t_])
            returns[:, t_] = G
        ...
        for t in range(self.burn_in, T):
            ...
            # using ppo style updates since dont want the model to go far from teacher's prob dist
            # basically pushing awy from uncertainity
            log_probs_act = log_probs.gather(1, acts[:, t].unsqueeze(1)).squeeze(1)

            q_tar, _, _, _ = self.nnTarget(ss[:, t], hx, cx)
            log_probs_target = F.log_softmax(q_tar, dim=1)
            target_log_probs_act = log_probs_target.gather(1, acts[:, t].unsqueeze(1)).squeeze(1)

            ratio = torch.exp(log_probs_act - target_log_probs_act)
            value = self.model.value_head(z)
            value_loss = F.mse_loss(value.squeeze(1), returns[:, t], reduction='none')

            advantage = returns[:, t] - value.detach().squeeze(1)
            advantage = (advantage - advantage.mean()) / (advantage.std() + 1e-8)
            ppo_loss = -torch.min( ratio * advantage, torch.clamp(ratio, 1 - self.clip_eps, 1 + self.clip_eps) * advantage)
            ...
            with torch.no_grad():
                _, z_next, _, _ = self.model(sNexts[:, t], hx.detach(), cx.detach())

            z_pred = self.model.predict_next_latent(z, acts[:, t])
            forward_loss = F.mse_loss(z_pred, z_next, reduction='none').mean(dim=1)
