...
class D3QN_PER():
    ...
    def trainNetwork(self, episode, experiences):
        ...
        for t in range(self.burn_in, T):
            ...
            stuck_loss = 0.1 * stuck.squeeze(1).mean()

            # rewarding if sensor detects box
            extra_bonus_for_getting_attached_to_box_if_detected = 0.2 * (ss[:, t, 16] * ss[:, t, 4:12].mean(dim=1)).mean()
            progress_reward =  0.2 * torch.mean(ss[:, t,:16], dim=1) + extra_bonus_for_getting_attached_to_box_if_detected
