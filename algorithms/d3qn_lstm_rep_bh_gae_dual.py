...
import math
import time
from helper import decayEpsilon, huberLoss, compute_td_lambda_returns
...
class D3QN_PER():
    def __init__(self, env1, env2, config):
        ...
        self.clip_td = config.get('clip_td', False)
        ...
        self.time_decay_progress = config.get('time_decay_progress', False)
    ...
    def trainAgent(self):
        ...
        for episode in range(self.MAX_TRAIN_EPISODES):
            # Early Exit 
            if os.path.exists("STOP_TRAINING_LAMBDA"):
                ...
                os.remove("STOP_TRAINING_LAMBDA")
                break
            ...
            while not done:
                ...
                if self.rBuffer.length() >= self.minSamples and (totalSteps + 1) % 4 == 0:
                    batch_experiences, indices, batch_weights, start_indices = self.rBuffer.sample(running_step=totalSteps, priority_seq_sampling=True)
                    ...
                    self.trainNetwork(episode, experiences, global_steps)
    ...
    def trainNetwork(self, episode, experiences, global_steps=None):
        #Your code goes in here
        self.nnOnline.train()
        self.nnTarget.train()
        ...
        for t in range(self.burn_in, T):
            ...
            forward_loss_list.append(forward_loss)
            ...
            inverse_loss_list.append(inverse_loss)
            ...
            intrinsic_reward = intrinsic_reward / (intrinsic_reward.mean().detach() + 1e-8)
            ...
            stuck_loss_list.append(stuck_loss)

            # rewarding if sensor detects box
            progress_reward = 0.2 * torch.mean(ss[:, t,:16], dim=1)
            
            # penalizing if stuck and state not changing (since it attaches with the wall and searching in contact with it only)
            # and giving reward for alignment
            delta = torch.norm(sNexts[:, t, :16] - ss[:, t, :16], dim=1)
            front_contact = ss[:, t, 15]

            # penalty
            stuck_penalty = 0.25 * front_contact * torch.clamp(0.01 - delta, min=0)

            # alignment reward (smooth peak) [giving less reward for too small and too large change]
            alignment_bonus = 0.1 * front_contact * torch.exp(-50 * (delta - 0.03)**2)

            if self.time_decay_progress:
                time_decay_step = math.exp(-0.05 * (t - self.burn_in))
                time_decay_episode = math.exp(-0.002 * global_steps)
                time_decay = time_decay_step * time_decay_episode

                alignment_bonus = alignment_bonus * time_decay
                progress_reward = progress_reward * time_decay
            
            total_reward = rs_env[:,t] + max(0.01, eta) * intrinsic_reward + progress_reward - stuck_penalty + alignment_bonus
            ...
            rewards_seq.append(total_reward)
            max_q_seq.append(max_q)
            dones_seq.append(dones[:,t])
            q_vals_seq.append(q_val)
            # ----------------------------------------------------------
            
            # bump loss
            td_target_1 = total_reward + self.gamma * max_q.detach() * (1 - dones[:, t])
            td_error_1 = td_target_1 - q_val
            
            bump_target = (td_error_1.detach() < 0).float()
            bump_loss = F.binary_cross_entropy_with_logits(
                bump_bad_logit.squeeze(1),
                bump_target
            )
            bump_loss_list.append(bump_loss)
        ...
        rewards_seq_t = torch.stack(rewards_seq, dim=1)
        max_q_seq_t = torch.stack(max_q_seq, dim=1)
        dones_seq_t = torch.stack(dones_seq, dim=1)
        q_vals_seq_t = torch.stack(q_vals_seq, dim=1)

        td_lambda_targets = compute_td_lambda_returns(
            rewards_seq_t,
            max_q_seq_t,
            dones_seq_t,
            gamma=self.gamma,
        ...
        td_error = td_lambda_targets - q_vals_seq_t

        weights_exp = weights.unsqueeze(1)
        if self.loss_fn == 'HuberLoss':
            d3qn_loss = huberLoss(td_error, self.delta, weights_exp, norm=True)
        else:
            d3qn_loss = (weights_exp * td_error.pow(2)).sum() / weights_exp.sum()

        # ===== Auxiliary losses =====
        forward_loss = torch.stack(forward_loss_list, dim=0).mean()
        inverse_loss = torch.stack(inverse_loss_list, dim=0).mean()
        stuck_loss = torch.stack(stuck_loss_list, dim=0).mean()
        bump_loss = torch.stack(bump_loss_list, dim=0).mean()

        # total loss
        loss = (
            d3qn_loss + 
            self.alpha_f * forward_loss +
            self.beta_i * inverse_loss +
            0.3 * stuck_loss +
            0.1 * bump_loss
        )
        ...
        td_error_for_per = td_error
        if self.clip_td == True:
            td_error_for_per = torch.clamp(td_error.detach(), -5, 5)
        self.rBuffer.update(td_error_for_per, indices, start_indices)
