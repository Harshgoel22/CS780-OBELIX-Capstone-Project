...
class D3QN_PER():
    ...
    def trainAgent(self):
        ...
        for episode in range(self.MAX_TRAIN_EPISODES):
            ...
            while not done:
                action, hx, cx = self.explorationStrategyTrainFn(self.nnOnline, state, self.epsilons[episode], useLSTM=True, hx=hx, cx=cx)
    ...
    def trainNetwork(self, experiences):
        ...
        with torch.no_grad():
            for t in range(self.burn_in):
                _, hx, cx = self.nnOnline(ss[:, t], hx, cx)

        for t in range(self.burn_in, T):
            qs, hx, cx = self.nnOnline(ss[:,t], hx, cx)
            qs_next, _, _ = self.nnOnline(sNexts[:,t], hx.detach(), cx.detach())
            a_next = qs_next.argmax(dim=1)

            qt_next, _, _ =  self.nnTarget(sNexts[:,t], hx.detach(), cx.detach())
    ...
    def evaluateAgent(self):
        ...
        for e in range(self.MAX_EVAL_EPISODES):
            ...
            while not done:
                action, hx, cx = self.explorationStrategyEvalFn(self.nnOnline, state, useLSTM=True, hx=hx, cx=cx)
                next_state, reward, done = self.env.step(self.actions[action], render=False)
