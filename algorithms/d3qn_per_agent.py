...
import random
import time
from helper import createDuelingNetwork, decayEpsilon, huberLoss
...
class D3QN_PER():
    def __init__(self, env, config):
        ...
        # it will be a string namely 'MSE', 'HuberLoss'
        self.loss_fn = config['loss_fn']

        self.model_path = config['model_path']

        # creating Q network
        self.nnTarget = createDuelingNetwork(stateDim, actionDim, hDim = config['hDim']).to(self.device)
        self.nnOnline = createDuelingNetwork(stateDim, actionDim, hDim = config['hDim']).to(self.device)
        ...
        self.rBuffer = ReplayBuffer(self.bufferSize, bufferType='PER-D3QN', alpha=self.alpha, beta=self.beta, beta_rate=self.beta_rate)
    ...
    def trainAgent(self):
        ...
        for episode in range(self.MAX_TRAIN_EPISODES):
            ...
            while not done:
                ...
                if self.rBuffer.length() >= self.batchSize and totalSteps % 5 == 0:
                    experiencesList, indices, weights = self.rBuffer.sample(self.batchSize, running_step=totalSteps)
                    experiences = (experiencesList, indices, weights)
            ...
            """
            # submission 3 nd 4: updating based on episodes
            if (episode+1) % self.updateFrequency == 0:
                self.updateNetwork(self.nnOnline, self.nnTarget)
            """
            
            if (episode+1) % 10 == 0:
                torch.save(self.nnOnline.state_dict(), f"{self.model_path}/d3qn_per_weights_{episode+1}.pth")
    ...
    def trainNetwork(self, experiences):
        ...
        self.nnOnline.train()
        self.nnTarget.train()
        experiencesList, indices, weights = experiences
        ss, acts, rs, sNexts, dones = self.rBuffer.splitExperiences(experiencesList)
        ...
        if self.loss_fn == 'HuberLoss':
            loss = huberLoss(tdErrors, self.delta, weights)
        else:
        ...
        new_priorities = torch.abs(tdErrors).detach().cpu().numpy() + 1e-6
        self.rBuffer.update(indices, new_priorities)
