...
import random
import time
from helper import createValueNetwork, decayEpsilon
...
class DDQN():
    def __init__(self, env, config):
        ...
        # creating Q network
        self.nnTarget = createValueNetwork(stateDim, actionDim, hDim = [64, 64]).to(self.device)
        self.nnOnline = createValueNetwork(stateDim, actionDim, hDim = [64, 64]).to(self.device)
    ...
    def trainAgent(self):
        ...
        for episode in range(self.MAX_TRAIN_EPISODES):
            ...
            if (episode+1) % 10 == 0:
                torch.save(self.nnOnline.state_dict(), f"../model_weights_phase1_sub1/ddqn_weights_{episode+1}.pth")
