...
class D3QN_PER():
    ...
    def trainAgent(self):
        ...
        for episode in range(self.MAX_TRAIN_EPISODES):
            ...
            if (episode+1) % 20 == 0:
                torch.save(self.nnOnline.state_dict(), f"{self.model_path}/lstm_d3qn_rep_w_{episode+1}.pth")
    ...
    def trainNetwork(self, experiences):
        ...
        with torch.no_grad():
            for t in range(self.burn_in):
                _, _, hx, cx = self.nnOnline(ss[:, t], hx, cx)

        for t in range(self.burn_in, T):
            qs, z_t, hx, cx = self.nnOnline(ss[:,t], hx, cx)

            hx_next = hx.detach().clone()
            cx_next = cx.detach().clone()
            with torch.no_grad():
                _, z_next, _, _ = self.nnOnline(sNexts[:,t], hx_next, cx_next)
            ...
            total_reward = rs_env[:,t] + self.eta * intrinsic_reward

            # D3QN Loss
            qs_next, _, _, _ = self.nnOnline(sNexts[:, t], hx.detach(), cx.detach())
