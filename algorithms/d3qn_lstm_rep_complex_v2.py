...
class D3QN_PER():
    ...
    def trainNetwork(self, episode, experiences):
        ...
        for t in range(self.burn_in, T):
            ...
            eta_current = self.eta * max(0.05, 1.0 - (episode - self.minSamples) / (self.MAX_TRAIN_EPISODES * 0.7))
            total_reward = rs_env[:,t] + eta_current * intrinsic_reward - 0.1 * bad_state_target
