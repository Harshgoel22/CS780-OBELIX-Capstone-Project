...
class D3QN_PER():
    ...
    def trainNetwork(self, episode, experiences):
        ...
        for t in range(self.burn_in, T):
            ...
            """
            sign = torch.where(rs_env[:, t] <= 0, -1.0, 1.0)
            intrinsic_reward = (intrinsic_reward + 1e-3) * sign
            ...
            extra_bonus_for_getting_attached_to_box_if_detected = (bump.squeeze(1) *  ss[:, t, 16] * ss[:, t, 4:12].mean(dim=1) * (acts[:,t]==2).float()).mean()
