...
class D3QN_PER():
    ...
    def trainAgent(self):
        ...
        for episode in range(self.MAX_TRAIN_EPISODES):
            ...
            while not done:
                action, hx, cx = self.explorationStrategyTrainFn(self.nnOnline, state, self.epsilons[episode], hx=hx, cx=cx)
            ...
            if (episode + 1) % 20 == 0:
                ...
                print(f"Episode {episode+1}: TR {self.trainRewardsList[-1]:.3f} | ER {self.evalRewardsList[-1]:.3f} | TT {self.trainTimeList[-1]:.3f} | WC {self.wallClockTimeList[-1]:.3f} | TS {self.totalStepsList[-1]:.0f}")
                torch.save(self.nnOnline.state_dict(), f"{self.model_path}/lstm_d3qn_rep_bad_head_w_{episode+1}.pth")

            else:
                print(f"Episode {episode+1}: TR {self.trainRewardsList[-1]:.3f} | TT {self.trainTimeList[-1]:.3f} | TS {self.totalStepsList[-1]:.0f}")
    ...
    def trainNetwork(self, experiences):
        ...
        for t in range(self.burn_in, T):
            ...
            bump = ss[:, t, 16]
            stuck = ss[:, t, 17]
            bad_state_target = torch.clamp(bump + stuck, 0, 1)
            bad_penalty = torch.sigmoid(bad_state_logit).detach().squeeze(1)
            bad_state_loss = F.binary_cross_entropy_with_logits(
            ...
            total_reward = rs_env[:,t] + self.eta * intrinsic_reward - 0.1 * bad_state_target
            ...
            td_target = total_reward + self.gamma * max_q.detach() * (1 - dones[:, t]) - 0.3 * bad_penalty
